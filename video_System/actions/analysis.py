...
def _parse_video_records(source):
    records = []
    for file_path in 查找视频文件(source, extensions={".mp4"}):
        stem = file_path.stem
        if "++=" not in stem:
...
def UP主统计分析(params):
    ...
    def runner():
        ...
        records = _parse_video_records(source)
...
def 关键词整理(params):
    ...
    def runner():
        ...
        moved = 0
        for file_path in 查找视频文件(source):
...
def Excel映射重命名(params):
    ...
    def runner():
        ...
        processed = []
        for file_path in folder_path.rglob("*.mp4"):
